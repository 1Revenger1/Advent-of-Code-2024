...
pub fn solve(str: String) -> SolutionPair {
    ...
    for l in str.lines() {
        ...
        let l = comps.entry(l_name).or_insert(Computer{ name: l_name, conns: HashSet::new() });
        l.conns.insert(r_name);
        let r = comps.entry(r_name).or_insert(Computer{ name: r_name, conns: HashSet::new() });
        ...
        for first_conn in &first.conns {
            ...
            for second_conn in &second.conns {
                ...
                if third.conns.contains(*name) {
                    if !name.starts_with('t') && !second.name.starts_with('t') && !third.name.starts_with('t') {
                        continue;
                    }
