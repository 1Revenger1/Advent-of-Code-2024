...
pub fn solve(str: String) -> SolutionPair {
    ...
    let instructions: Vec<u64> = instructions[9..].split(',').map(|str| str.trim().parse().unwrap()).collect();
    ...
    let sol1 = run(cpu, &instructions).iter().map(|v| v.to_string()).collect::<Vec<String>>().join(",");
