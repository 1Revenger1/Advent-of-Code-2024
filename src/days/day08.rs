    ...
    for (y, l) in str.lines().enumerate() {
        for (x, c) in l.chars().enumerate() {
            if c.is_alphanumeric() {
                nodes.entry(c).or_default().push(Coordinates(x as i32, y as i32));
            }
        }
    }
    ...
    nodes.values().for_each(|n| {
        for i in 0..n.len() {
            for j in 0..n.len() {
                ...
                if new_x < 0 || new_x >= max_size.0 ||
                    new_y < 0 || new_y >= max_size.1
                {
                    continue;
                }
        ...
        for i in 0..n.len() {
            for j in 0..n.len() {
                ...
                loop {
                    ...
                    if next.0 < 0 || next.0 >= max_size.0 ||
                        next.1 < 0 || next.1 >= max_size.1
                    {
                        break;
                    }
