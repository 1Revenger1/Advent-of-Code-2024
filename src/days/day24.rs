...
fn get_z(queue: &VecDeque<&str>, bit_vals: &HashMap<&str, bool>, results: &HashMap<&str, HashSet<&str>>, ops: &HashMap<&str, (OP, &str, &str)>) -> u64 {
    ...
    while !queue.is_empty() {
        ...
        for &d in dest {
            if bit_vals.contains_key(d) { continue; }
            let (op, left, right) = ops[d];
            let left = if let Some(t) = bit_vals.get(left) { t } else { continue; };
            let right = if let Some(t) = bit_vals.get(right) { t } else { continue; };
            bit_vals.insert(d, match op {
                OP::AND => left & right,
                OP::XOR => left ^ right,
                OP::OR => left | right
            });
...
pub fn solve(str: String) -> SolutionPair {
    ...
    for z in 2..45 {
        ...
        if right.starts_with("x") || right.starts_with("y") || left.starts_with("x") || left.starts_with("y") {
            continue;
        }
    ...
    for (n, (o, l, r)) in &ops {
        ...
        if n.starts_with("z") || l.starts_with("x") || l.starts_with("y") || r.starts_with("x") || r.starts_with("y")  {
            continue;
        }
