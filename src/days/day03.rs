...
fn p1(str: &str) -> i32 {
    ...
    for i in 0..str.len() {
        ...
        let num1 = iter.clone().take_while(|c| c.is_numeric()).collect::<String>();
        if num1 == "" {
            continue;
        }
        ...
        let num2 = iter.clone().take_while(|c| c.is_numeric()).collect::<String>();
        if num2 == "" {
            continue;
        }
...
fn get_num_arg(iter: &mut Chars) -> Result<i32, CompIntParseError> {
    let arg = iter.clone().take_while(|c| c.is_numeric()).collect::<String>();
    if arg == "" {
