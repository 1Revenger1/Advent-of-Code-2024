...
pub fn solve(str: String) -> SolutionPair {
    let codes: Vec<(i64, Vec<Coordinates>)> = str.lines().map(|l| {
        (l[0..3].parse().unwrap(), l.chars().map(|c| get_coord_from_numpad(c)).collect())
    }).collect();

    let sol1: i64 = codes.iter().map(|c| {
        let numpad = p1(&c.1, Pad::NumPad);
        let numpad: Vec<Coordinates> = numpad.iter().map(|d| get_coord_from_dirpad(*d).unwrap()).collect();
        let one = p1(&numpad, Pad::DirPad);
        let one: Vec<Coordinates> = one.iter().map(|d| get_coord_from_dirpad(*d).unwrap()).collect();
    ...
    let sol2: i64 = codes.iter().map(|c| {
        ...
        for _ in 0..25 {
            ...
            for (subset, count) in dirs {
                let subset = subset.iter().map(|d| get_coord_from_dirpad(*d).unwrap()).collect();
                let dirpad = p2(&subset, Pad::DirPad);
                for (r, v) in dirpad {
                    new_dirs.entry(r).and_modify(|e| *e += v * count).or_insert(v * count);
                }
            }
            dirs = new_dirs;
        }

        c.0 * dirs.iter().map(|(subset, count)| subset.len() as i64 * *count).sum::<i64>()
    }).sum();
